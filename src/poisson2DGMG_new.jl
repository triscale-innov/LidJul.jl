...
function PoissonGMG_new(l2D,::Type{SMOOTHER};nlevels=-1,nprae=2,npost=1,ncoarse=20) where {SMOOTHER}
    Lx,Ly=l2D.Lx,l2D.Ly
    nx,ny=l2D.nx,l2D.ny
    ...
    PoissonGMG_new{SMOOTHER}(Sol,Solb,RHS,invh2,smoothers,nlevels,nprae,npost,ncoarse,bc)
end
...
function img_solver(nlevels,res_1,g::PoissonGMG_new)
    res=res_1
    res_old=0.
...
function smooth(lev,g::PoissonGMG_new)
...
function interpolate_correct(lev,g::PoissonGMG_new)
    uf=g.Sol[lev-1]
    uc=g.Sol[lev]
    (nrows,ncols)=size(uc)
    ncm1=ncols-1
