export PoissonSparsePCG,solve!
using IterativeSolvers
using Preconditionners
using IncompleteLU

struct PoissonSparsePCG{Preco}
    spLxy::SparseMatrixCSC{Float64,Int64}
    p::Preco
    iluLxy::Any

    function PoissonSparsePCG(sp)
        tilu=@elapsed lu = ilu(sp, τ = 0.2)
        println("ILU factorization time:",tilu)
        new(sp,lu)
    end
end

function solve!(Xxy,Bxy,sp::PoissonSparsePCG)
    p = sp.iluLxy
    b=view(Bxy,1:length(Bxy))
    x=view(Xxy,1:length(Xxy))
    a,cgilulog=IterativeSolvers.cg!(x,sp.spLxy,b,Pl = p;log=true,tol = 1e-10,maxiter=1000)
    @show cgilulog
    cgilulog
end
