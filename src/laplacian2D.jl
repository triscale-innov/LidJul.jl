...
function SparseArrays.sparse(a::Laplacian2D)
    ...
    for i=1:nx
        for j=1:ny
            I=i+nx*(j-1)
            push_element!(Is,Js,Vs,I,I,lpx[i,i]+lpy[j,j])
            i>1  && push_element!(Is,Js,Vs,I,I-1,lpx[i,i-1])
            i<nx && push_element!(Is,Js,Vs,I,I+1,lpx[i,i+1])
            j>1 &&  push_element!(Is,Js,Vs,I,I-ny,lpy[j,j-1])
            j<ny && push_element!(Is,Js,Vs,I,I+ny,lpy[j,j+1])
        end
    end
    sp=sparse(Is,Js,Vs)
    sp
